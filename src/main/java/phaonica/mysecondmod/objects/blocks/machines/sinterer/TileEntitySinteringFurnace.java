...
public class TileEntitySinteringFurnace extends TileEntity implements IInventory, ITickable
{
	...
	public void update()
	{
		...
		if(!this.world.isRemote)
		{
			...
			ItemStack stack = (ItemStack)this.inventory.get(2); // fuel
			
			// if furnace is burning OR fuel slot is empty AND input1 or input2 are empty
			...
			else if ( !this.isBurning() && this.cookTime > 0 ) // if you run out of fuel halfway through cooking, set progress to zero
			{
	...
	{
		return 200; // how fast this furnace cooks // lower number increases speed
	}
	...
	public static int getItemBurnTime(ItemStack fuel)
	{
		...
		else
		{
			...
			if ( item instanceof ItemBlock && Block.getBlockFromItem(item) != Blocks.AIR)
			{
				...
				if(block == Blocks.DOUBLE_WOODEN_SLAB) return 150;
			...
			if (item instanceof ItemHoe && "WOOD".equals(((ItemSword)item).getToolMaterialName())) return 200;
	...
	{
		return "phaomsm:sintering_furnace";
	}
