package phaonica.mysecondmod.util;

public class Reference {

	public static final String MOD_ID = "phaomsm";
	public static final String MOD_NAME = "My Second Mod Name";	
	public static final String MOD_VERSION = "0.1 - MC 1.12.2";

	public static final String CLIENTPROXY = "phaonica.mysecondmod.proxy.ClientProxy";
	public static final String COMMONPROXY = "phaonica.mysecondmod.proxy.CommonProxy";
	
	public static final int ENTITY_CENTAUR = 120;
	public static final int GUI_SINTERING_FURNACE = 0;
}
